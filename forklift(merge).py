...
if uploaded_file is not None and 'df' in locals():
    def generate_pivot(month, department, process, forklift_class, workplace):
        def calculate_average_by_time(filtered_df, value_name):
            return filtered_df.groupby('시간대')[value_name].mean().round(2)
        filtered_df = df.copy()
        if month != '전체':
    ...
    # 시간대별 평균 계산
    if analysis_type == '운영 대수':
        avg_by_time = calculate_average_by_time(df, '차대 코드')
        avg_summary_text = f"<b>시간대별 평균 운영 대수</b><br>" + "<br>".join([f"시간대 {time}: 평균 {value}대" for time, value in avg_by_time.items()])
    else:
        avg_by_time = calculate_average_by_time(df, '운영 시간(초)')
        avg_summary_text = f"<b>시간대별 평균 운영 시간</b><br>" + "<br>".join([f"시간대 {time}: 평균 {value}초" for time, value in avg_by_time.items()])

    # 요약 정보를 히트맵 아래로 표시
    st.markdown(avg_summary_text, unsafe_allow_html=True)
