    ...
    def generate_pivot(month, department, process, forklift_class, workplace):
        ...
        if analysis_type == '운영 대수':
            ...
            summary = {
                ...
                'max_avg_operating_units_per_time': f"{max_avg_units_time} {max_avg_units_value}대"
            }
        else:
            index_name = '차대 코드'
            value_name = '시작 날짜'
            ...
            avg_time_per_time = filtered_df.groupby('시간대')['운영 시간(초)'].mean().max()
            max_avg_time_time = filtered_df.groupby('시간대')['운영 시간(초)'].mean().idxmax()
            max_avg_time_value = filtered_df.groupby('시간대')['운영 시간(초)'].mean().max()
            ...
            min_operating_time_formatted = format_time(min_oper
