...
with st.sidebar:
    ...
    if uploaded_file is not None:
        try:
            ...
            if missing_required: st.stop()
            else: df = df_initial.copy()
            if missing_optional:
                for col in missing_optional: df[col] = '정보 없음'
            try:
        ...
        except (pd.errors.EmptyDataError, FileNotFoundError): df = None
...
def generate_pivot(original_df, month, department, process, forklift_class, workplace, analysis_type):
    # ... (generate_pivot 함수 내용은 이전과 동일하게 유지) ...
    ...
    if analysis_type == '운영 대수':
        ...
        if not pivot_table_result.empty:
            try:
                total_operating_units = filtered_df[value_name].nunique(); daily_counts = filtered_df.groupby('시작 날짜_표시용')[value_name].nunique(); min_operating_units = daily_counts.min() if not daily_counts.empty else 0; max_operating_units = daily_counts.max() if not daily_counts.empty else 0; min_operating_day = daily_counts.idxmin() if not daily_counts.empty and min_operating_units > 0 else '데이터 없음'; max_operating_day = daily_counts.idxmax() if not daily_counts.empty and max_operating_units > 0 else '데이터 없음'; avg_operating_units = round(daily_counts.mean()) if not daily_counts.empty else 0; min_units_ratio = (min_operating_units / total_operating_units * 100) if total_operating_units > 0 else 0; max_units_ratio = (max_operating_units / total_operating_units * 100) if total_operating_units > 0 else 0; avg_units_ratio = (avg_operating_units / total_operating_units * 100) if total_operating_units > 0 else 0
            ...
            except Exception: local_summary = {}
    elif analysis_type == '운영 횟수':
        index_name = '차대 코드'; title = f'지게차 시간대별 운영 횟수 ({selected_month}월)' if selected_month != '전체' else '지게차 시간대별 운영 횟수 (전체 월)'; pivot_table_counts = pd.DataFrame(); pivot_table_times = pd.DataFrame()
        try:
            pivot_table_counts = filtered_df.pivot_table(index=index_name, columns='시간대', values='시작 날짜', aggfunc='count').fillna(0); pivot_table_times = filtered_df.pivot_table(index=index_name, columns='시간대', values='운영 시간(초)', aggfunc='sum').fillna(0)
            if not pivot_table_counts.empty: pivot_table_counts = pivot_table_counts.sort_index(axis=1); pivot_table_times = pivot_table_times.reindex(index=pivot_table_counts.index, columns=pivot_table_counts.columns).fillna(0)
            pivot_data['counts'] = pivot_table_counts; pivot_data['times'] = pivot_table_times
        except Exception: return {}, title, index_name, {}
        if not pivot_table_counts.empty:
            try:
                 unit_counts = filtered_df.groupby('차대 코드')['시작 날짜'].count(); total_operating_counts = unit_counts.sum(); min_operating_counts = unit_counts.min() if not unit_counts.empty else 0; max_operating_counts = unit_counts.max() if not unit_counts.empty else 0; min_operating_unit = unit_counts.idxmin() if not unit_counts.empty and min_operating_counts > 0 else '데이터 없음'; max_operating_unit = unit_counts.idxmax() if not unit_counts.empty and max_operating_counts > 0 else '데이터 없음'; avg_operating_counts = round(unit_counts.mean()) if not unit_counts.empty else 0; min_counts_ratio = (min_operating_counts / total_operating_counts * 100) if total_operating_counts > 0 else 0; max_counts_ratio = (max_operating_counts / total_operating_counts * 100) if total_operating_counts > 0 else 0; avg_counts_ratio = (avg_operating_counts / total_operating_counts * 100) if total_operating_counts > 0 else 0
                 operating_times = filtered_df.groupby('차대 코드')['운영 시간(초)'].sum(); total_operating_time = operating_times.sum(); min_operating_time = operating_times.min() if not operating_times.empty else 0; max_operating_time = operating_times.max() if not operating_times.empty else 0; min_time_unit = operating_times.idxmin() if not operating_times.empty and min_operating_time > 0 else '데이터 없음'; max_time_unit = operating_times.idxmax() if not operating_times.empty and max_operating_time > 0 else '데이터 없음'; avg_operating_time = operating_times.mean() if not operating_times.empty else 0; min_time_ratio = (min_operating_time / total_operating_time * 100) if total_operating_time > 0 else 0; max_time_ratio = (max_operating_time / total_operating_time * 100) if total_operating_time > 0 else 0; avg_time_ratio = (avg_operating_time / total_operating_time * 100) if total_operating_time > 0 else 0
    ...
    if pivot_data and isinstance(pivot_data, dict):
        ...
        if pivot_table is not None and not pivot_table.empty:
            ...
            for r in range(len(pivot_table.index)):
                y_label = pivot_table.index[r]; row_tooltips = []
                for c in range(len(pivot_table.columns)):
                    x_label = pivot_table.columns[c]; value = pivot_table.iloc[r, c]
                    ...
                    if analysis_type == '운영 횟수' and pivot_table_times is not None:
                        try: time_value = pivot_table_times.iloc[r, c]; formatted_time = format_time(time_value); cell_tooltip += f"<br>사용 시간: {formatted_time}"
                        except (IndexError, Exception): cell_tooltip += "<br>사용 시간: -"
            ...
            # --- 최댓값 하이라이트 추가 (진단 코드 포함) ---
            if pivot_table.values.size > 0:
                st.write("--- [Max Highlight Debug Start] ---") # Debug Start
                try:
                    st.write(f"Checking pivot table with shape: {pivot_table.values.shape}") # Debug Shape
                    numeric_values = pd.to_numeric(pivot_table.values.flatten(), errors='coerce')
                    # st.write(f"Numeric values (first 10): {numeric_values[:10]}") # 너무 길어질 수 있으므로 주석 처리
                    valid_values = numeric_values[~np.isnan(numeric_values)]
                    st.write(f"Number of valid numeric values: {valid_values.size}") # Debug Valid Count

                    if valid_values.size > 0:
                        max_value = valid_values.max()
                        st.write(f"Calculated max_value: {max_value}") # Debug Calculated Max

                        if max_value > 0: # 0보다 클 때만 표시
                            st.write("Max value is > 0. Finding indices...") # Debug Condition Met
                            # NaN 값 비교를 안전하게 하기 위해 isclose 대신 직접 비교 사용 (이미 numeric 변환 및 NaN 제거됨)
                            max_indices = np.where(pd.to_numeric(pivot_table.values, errors='coerce') == max_value)
                            st.write(f"Indices found (row, col): {max_indices}") # Debug Indices Found

                            if len(max_indices[0]) > 0:
                                st.write(f"Found {len(max_indices[0])} location(s) for max value. Adding trace(s)...") # Debug Plotting Start
                                ...
                                st.write("Trace(s) added.") # Debug Plotting End
                            else:
                                st.write("Indices array was empty. Highlight not plotted.") # Debug No Indices
                        else:
                            st.write("Max value is 0 or less. Highlight not plotted.") # Debug Max <= 0
                    else:
                        st.write("No valid numeric values found in pivot table. Highlight not plotted.") # Debug No Valid Values

                except Exception as e_debug:
                     # 오류 발생 시에도 메시지 출력 (진단 목적)
                     st.write(f"--- [Max Highlight Debug Error] ---")
                     st.write(f"An error occurred during max value highlighting: {e_debug}")
                     # pass # 최종 버전에서는 pass로 변경

                st.write("--- [Max Highlight Debug End] ---") # Debug End
            # --- 최댓값 하이라이트 끝 ---
