...
with st.sidebar:
    ...
    if analysis_options_enabled:
        ...
        # else: # 오류는 위에서 이미 처리됨
            # st.warning("데이터 로딩 중 문제가 발생하여 분석 옵션을 표시할 수 없습니다.")
...
if df is not None and not df.empty and analysis_options_enabled:
    ...
    def generate_pivot(month, department, forklift_class, workplace, analysis_type_local):
        ...
        if month != '전체':
            if pd.api.types.is_numeric_dtype(filtered_df['월']):
                 # NaT 값은 비교에서 자동으로 제외됨
                filtered_df = filtered_df[filtered_df['월'] == month]
            else:
                st.warning("'월' 컬럼 필터링 불가 (타입 오류)")
        if department != '전체':
        ...
        if analysis_type_local == '운영 대수':
            try:
                ...
                value_name = '차대 코드'
                agg_func = 'nunique'
                local_title = f"{month}월 {department} 지게차 일자별 운영 대수" if month != '전체' else f"{department} 지게차 일자별 운영 대수"
            ...
            try:
                ...
                agg_func = 'count'
                local_title = f"{month}월 {department} 지게차 차대별 시간대별 운영 횟수" if month != '전체' else f"{department} 지게차 차대별 시간대별 운영 횟수"

                 # null 값 없는지 확인 (만약 있다면 다른 컬럼 사용 고려)
                if filtered_df[value_name_for_count].isnull().any():
                    st.warning(f"'{value_name_for_count}' 열에 null 값이 있어 운영 횟수 집계에 영향이 있을 수 있습니다.")
                    # 필요 시 다른 열 선택 또는 null 처리
                    # value_name_for_count = '다른_널없는_컬럼'
        ...
        try:
            ...
            for r_idx, row_name in enumerate(pivot_table.index):
                ...
                for c_idx, col_name in enumerate(pivot_table.columns):
                    ...
                    y_label = f"{index_name}: {row_name}<br>"
                    x_label = f"시간대: {col_name}<br>"
                    val_label = f"{'운영 대수' if analysis_type == '운영 대수' else '운영 횟수'}: {int(val)}{'대' if analysis_type == '운영 대수' else '회'}"
                    row_texts.append(f"{y_label}{x_label}{val_label}")
            ...
            fig.update_layout(
                ...
                height=graph_height,
                # coloraxis_colorbar=dict(title='운영 강도') # Heatmap 생성 시 이미 설정됨
            )

            # y축 정렬 및 타입 설정
            if analysis_type == '운영 대수':
                 # 날짜 문자열을 datetime으로 변환하여 정렬 키 생성
                try:
                    sorted_dates = sorted(pivot_table.index, key=lambda d: pd.to_datetime(d, format='%m-%d'))
                    fig.update_yaxes(type='category', categoryorder='array', categoryarray=sorted_dates)
                except ValueError:
                    st.warning("Y축 날짜 정렬 중 오류 발생. 기본 순서로 표시합니다.")
            ...
            if summary: # summary 딕셔너리가 비어있지 않으면
                if analysis_type == '운영 대수':
                    ...
                    <div style="border: 1px solid #ddd; padding: 15px; border-radius: 5px; background-color: #f9f9f9;">
                        <h4 style="margin-top:0; color: #333;">일별 운영 대수 요약 ({selected_month}월 {selected_department})</h4>
                    ...
                    summary_html = f"""
                    <div style="border: 1px solid #ddd; padding: 15px; border-radius: 5px; background-color: #f9f9f9;">
                        <h4 style="margin-top:0; color: #333;">차대별 운영 요약 ({selected_month}월 {selected_department})</h4>
                        <div style="display: flex; justify-content: space-around; flex-wrap: wrap; gap: 15px;">
                            <div style="border-right: 1px solid #eee; padding-right: 15px; margin-right: 15px;">
                ...
                st.info("요약 정보를 생성할 수 없습니다 (데이터 부족 또는 오류 발생).")
    ...
    # 피벗 테이블이 비어있는 경우 (generate_pivot 함수에서 경고 후 빈 df 반환 시)
         ...
         if title != "분석 오류" and title != "분석 대기 중...": # generate_pivot 함수가 정상 실행됐으나 데이터가 없는 경우
             st.info("선택된 조건에 해당하는 데이터가 없어 그래프를 표시할 수 없습니다. 필터 조건을 변경해보세요.")
