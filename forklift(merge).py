...
if uploaded_file is not None and 'df' in locals():
    ...
    tooltip_texts = [[f'월 최대 운영 횟수: {int(val)}회' if analysis_type == '운영 횟수' else f'운영 대수: {int(val)}대' for val in row] for row in pivot_table.values]
    heatmap = go.Heatmap(
    ...
    if max_indices:
        ...
        for y_idx, x_idx in zip(max_y_indices, max_x_indices):
            fig.add_trace(go.Scatter(
                ...
                text=[f'{"동시 최대 투입:" if analysis_type == "운영 대수" else "시간대 최대 운영(월):"} {int(max_value)} {"대" if analysis_type == "운영 대수" else "회"}'],
