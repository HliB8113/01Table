...
if uploaded_file is not None and 'df' in locals():
    ...
    if analysis_type == '운영 대수':
        ...
        summary_text = (
            f"<b>운영 횟수 전체: {summary.get('total_counts', 'N/A')}번</b> &nbsp;&nbsp;&nbsp; "
            f"일일 최소 운영: {summary.get('min_counts_unit', 'N/A')} {summary.get('min_counts', 'N/A')}번 ({float(summary.get('min_counts_ratio', 0)):0.2f}%) &nbsp;&nbsp;&nbsp; "
            f"일일 최대 운영: {summary.get('max_counts_unit', 'N/A')} {summary.get('max_counts', 'N/A')}번 ({float(summary.get('max_counts_ratio', 0)):0.2f}%) &nbsp;&nbsp;&nbsp; "
            f"일일 평균 운영: {summary.get('avg_counts', 'N/A')}번 ({float(summary.get('avg_counts_ratio', 0)):0.2f}%)<br>"
            f"<b>운영 시간 전체: {summary.get('total_time', 'N/A')}</b> &nbsp;&nbsp;&nbsp; "
            f"일일 최소 운영: {summary.get('min_time_unit', 'N/A')} {summary.get('min_time', 'N/A')} ({float(summary.get('min_time_ratio', 0)):0.2f}%) &nbsp;&nbsp;&nbsp; "
            f"일일 최대 운영: {summary.get('max_time_unit', 'N/A')} {summary.get('max_time', 'N/A')} ({float(summary.get('max_time_ratio', 0)):0.2f}%) &nbsp;&nbsp;&nbsp; "
